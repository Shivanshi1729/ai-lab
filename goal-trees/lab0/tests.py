...
def count_pattern_3_getargs():
    ...
    answers['count_pattern_3_random'] = [[1, 2, 3, 2, 3],
                                         [1, 2, 3, 2, 3]*answers['count_pattern_3_random']]
...
def tree_ref_3_getargs():
    answers['tree_ref_3_random'] = [sample_tree,
                                    [random.randint(0, len(sample_tree)-1)]]
...
def distribution_5_getargs():
    answers['distribution_5_random'] = [encode_sumprod(
        Sum([2*random.randint(1, 50), Product([3, Product([8, Sum(['x', 'y']), 5])])]))]
